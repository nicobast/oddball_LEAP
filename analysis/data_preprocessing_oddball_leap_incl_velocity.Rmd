...
- definition of saved data sets:
  - 1. mmn_leap_merged_04072024.Rds - merged data before epochs
  - 2. mmn_leap_epoched_04072024.Rds - epoched data before gaze & pupil preprocessing
  - 3. mmn_leap_epochs_preprocessed_11072024.Rds - preprocessed data on per sample level to test new metrics
  - 4. mmn_leap_epochs_agg_11072024.Rds - preprocessed data aggregated per trial (state: July 2024)
  - 5. mmn_leap_pd_final_dfs_epoch_19072024 - final data used in analysis script
...
# --> test preprocessed and epoched data
...
## plot pupillary response --> USED FOR FIGURE 2
...
## function to retrieve pupillary change velocity response - INCREASE ####
fun_rpd_velocity_increase<-function(x){

  y<-x[x$time_epoch<=1.25,] #restrict to increase time frame
  rpd_velocity_increase<-mean(diff(y$rpd)/diff(y$time_epoch),na.rm=T)
  return(rpd_velocity_increase)

}

#apply function to list (TRIAL split list)
list_rpd_velocity_increase<-pblapply(df_list_epoch,function(x,y){sapply(x,fun_rpd_velocity_increase)})


## function to retrieve pupillary change velocity response - RECOVERY ####
fun_rpd_velocity_recovery<-function(x){

  y<-x[x$time_epoch>1.25 & x$time_epoch<2.5,] #restrict to increase time frame
  rpd_velocity_recovery<-mean(diff(y$rpd)/diff(y$time_epoch),na.rm=T)
  return(rpd_velocity_recovery)

}

#apply function to list (TRIAL split list)
list_rpd_velocity_recovery<-pblapply(df_list_epoch,function(x,y){sapply(x,fun_rpd_velocity_recovery)})
...
list_trial<-pbmapply(data.frame,list_trial,list_rpd_velocity_increase,SIMPLIFY=F)
list_trial<-pblapply(list_trial,fun_rename,variable_position=ncol(list_trial[[1]]),
                     new_name='rpd_velocity_increase') #rename LAST variable

list_trial<-pbmapply(data.frame,list_trial,list_rpd_velocity_recovery,SIMPLIFY=F)
list_trial<-pblapply(list_trial,fun_rename,variable_position=ncol(list_trial[[1]]),
                     new_name='rpd_velocity_recovery') #rename LAST variable
...
# on per participant aggregated ####
df_et_vars<-aggregate(df_epoch[,c('center_dev','pd','pd_baseline',
                                  'pd_res','rpd_response',
                                  'rpd_velocity_increase','rpd_velocity_recovery')],
                      by=list(df_epoch$subjects),FUN=mean,na.rm=T)
...
df_et_vars_rpd_velocity_increase<-aggregate(df_epoch[,c('rpd_velocity_increase')],
                              by=list(df_epoch$subjects,df_epoch$EventData_epoch),FUN=mean,na.rm=T)

df_et_vars_rpd_velocity_increase<-reshape(df_et_vars_rpd_velocity_increase, idvar = "Group.1", timevar = "Group.2", direction = "wide") #long to wide format

df_et_vars_rpd_velocity_recovery<-aggregate(df_epoch[,c('rpd_velocity_recovery')],
                              by=list(df_epoch$subjects,df_epoch$EventData_epoch),FUN=mean,na.rm=T)

df_et_vars_rpd_velocity_recovery<-reshape(df_et_vars_rpd_velocity_recovery, idvar = "Group.1", timevar = "Group.2", direction = "wide") #long to wide format

names(df_et_vars)[1]<-'subjects'
names(df_et_vars_rpd)[1]<-'subjects'
names(df_et_vars_rpd_res)[1]<-'subjects'
names(df_et_vars_rpd_velocity_increase)[1]<-'subjects'
names(df_et_vars_rpd_velocity_recovery)[1]<-'subjects'
...
df_dem<-merge(df_dem,df_et_vars_rpd_velocity_increase,by='subjects')
df_dem<-merge(df_dem,df_et_vars_rpd_velocity_recovery,by='subjects')


###on df_timepoint ####
df_et_vars<-aggregate(df_epoch[,c('center_dev','pd','pd_baseline',
                                  'pd_res','rpd_velocity_increase','rpd_velocity_recovery')],
...
df_et_vars_rpd_velocity_increase<-aggregate(df_epoch[,c('rpd_velocity_increase')],
                                            by=list(df_epoch$id,df_epoch$EventData_epoch),FUN=mean,na.rm=T)
df_et_vars_rpd_velocity_increase<-reshape(df_et_vars_rpd_velocity_increase, idvar = "Group.1", timevar = "Group.2", direction = "wide") #long to wide format

df_et_vars_rpd_velocity_recovery<-aggregate(df_epoch[,c('rpd_velocity_recovery')],by=list(df_epoch$id,df_epoch$EventData_epoch),FUN=mean,na.rm=T)
df_et_vars_rpd_velocity_recovery<-reshape(df_et_vars_rpd_velocity_recovery, idvar = "Group.1", timevar = "Group.2", direction = "wide") #long to wide format

names(df_et_vars)[1]<-'id'
names(df_et_vars_rpd)[1]<-'id'
names(df_et_vars_rpd_res)[1]<-'id'
names(df_et_vars_rpd_velocity_increase)[1]<-'id'
names(df_et_vars_rpd_velocity_recovery)[1]<-'id'
...
df_timepoint<-merge(df_timepoint,df_et_vars_rpd_velocity_increase,by='id')
df_timepoint<-merge(df_timepoint,df_et_vars_rpd_velocity_recovery,by='id')
...
#epoched data + velocity
save(df_epoch,df_dem,df_timepoint,
     file="C:/Users/nico/PowerFolders/project_oddball_LEAP/data/mmn_leap_pd_final_dfs_epoch_16092024")
...
## association of dependent variables

```{r}

with(df_timepoint,cor.test(pd_res,rpd_velocity_increase.201))
with(df_timepoint,cor.test(pd_res,rpd_velocity_increase.202))
with(df_timepoint,cor.test(pd_res,rpd_velocity_increase.203))
with(df_timepoint,cor.test(pd_res,rpd_velocity_increase.204))

with(df_timepoint,cor.test(pd_res,rpd_velocity_recovery.201))
with(df_timepoint,cor.test(pd_res,rpd_velocity_recovery.202))
with(df_timepoint,cor.test(pd_res,rpd_velocity_recovery.203))
with(df_timepoint,cor.test(pd_res,rpd_velocity_recovery.204))
...
lmm<-lmer(scale(rpd_velocity_increase)~EventData_epoch+
            (1|id),data=df_epoch[rpd_velocity_increase>-1 & rpd_velocity_increase<1],)
...
#BIC Bayes Factor
lmm_ML<-lmer(scale(rpd_velocity_increase)~EventData_epoch+
              (1|id),data=df_epoch[rpd_velocity_increase>-1 & rpd_velocity_increase<1],REML=F)

null_lmm_ML <- lmer(scale(rpd_velocity_increase)~1+
                  (1|id),data=df_epoch[rpd_velocity_increase>-1 & rpd_velocity_increase<1],REML=F)
...
lmm<-lmer(scale(rpd_velocity_recovery)~EventData_epoch+
            (1|id),data=df_epoch[rpd_velocity_recovery>-1 & rpd_velocity_recovery<1],)
...
#BIC Bayes Factor
lmm_ML<-lmer(scale(rpd_velocity_recovery)~EventData_epoch+
              (1|id),data=df_epoch[rpd_velocity_recovery>-1 & rpd_velocity_recovery<1],REML=F)

null_lmm_ML <- lmer(scale(rpd_velocity_recovery)~1+
                  (1|id),data=df_epoch[rpd_velocity_recovery>-1 & rpd_velocity_recovery<1],REML=F)
...
###polynomial fit of trial counter
    linear_fit<-lmer(scale(rpd_velocity_increase)~scale(EventCounter_epoch)+
            (1|id),data=df_epoch,REML=F)
  
    quadratic_fit<-lmer(scale(rpd_velocity_increase)~stats::poly(scale(EventCounter_epoch),2)+
                (1|id),data=df_epoch,REML=F)
  
    cubic_fit<-lmer(scale(rpd_velocity_increase)~stats::poly(scale(EventCounter_epoch),3)+
                 (1|id),data=df_epoch,REML=F)
...
lmm<-lmer(scale(rpd_velocity_increase)~EventCounter_epoch+
              (1|id),data=df_epoch)
...
###polynomial fit of trial counter
    linear_fit<-lmer(scale(rpd_velocity_recovery)~scale(EventCounter_epoch)+
            (1|id),data=df_epoch,REML=F)
  
    quadratic_fit<-lmer(scale(rpd_velocity_recovery)~stats::poly(scale(EventCounter_epoch),2)+
                (1|id),data=df_epoch,REML=F)
  
    cubic_fit<-lmer(scale(rpd_velocity_recovery)~stats::poly(scale(EventCounter_epoch),3)+
                 (1|id),data=df_epoch,REML=F)
...
lmm<-lmer(scale(rpd_velocity_recovery)~EventCounter_epoch+
              (1|id),data=df_epoch)
...
#MAIN MODEL - pupillary velocity by group - random intercept
lmm_REML<-lmer(scale(rpd_velocity_increase)~EventData_epoch*t1_diagnosis*EventCounter_epoch+
            (1|id),data=df_epoch[rpd_velocity_increase>-1 & rpd_velocity_increase<1,])
